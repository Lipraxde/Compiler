...
void push_symbol(void *data, int whitch_struct)
{
    ...
    if((whitch_struct==STRUCT_VARI)&&(now_level!=0))
    {
        struct variable_node *p = data;
        if(p->type->type != TEXT_TYPE) // String variable is not support
        {
            p->var_addr = now_locvaraddr;
            now_locvaraddr++;
        }
    }
...
void pop_symbol(int level)
{
    while((sym_count!=0)&&(sym_table[sym_count-1].level>level))
    {
        ...
        if((sym_table[sym_count].whitch_struct==STRUCT_VARI)&&
            (sym_table[sym_count].level!=0))
        {
            struct variable_node *p = sym_table[sym_count].data;
            if(p->type->type != TEXT_TYPE) // String variable is not support
                now_locvaraddr--;
        }
    }
...
void exit_level(void)
{
    ...
    if(deepest_addr < now_locvaraddr)
    {
        deepest_addr = now_locvaraddr;
    }
...
DUMP_BEGIN(func, function_node, "function declartion")
    while(p != 0)
    {
        ...
        // only arg and local variable
        p->loc_varcount = deepest_addr-1;
        deepest_addr = 0;
...
void dump_ast(void)
{
    ...
    ast->loc_varcount = deepest_addr-1;
    deepest_addr = 0;
/*
extern int linenum;
